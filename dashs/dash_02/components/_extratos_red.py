...
# Tabela
@app.callback(
    Output('table-entries-red', 'children'),
...
@app.callback(
    Output('bar-graph-entries-red', 'figure'),
    [Input('store-reds', 'data')]
)
def bar_graph(data):
    df = pd.DataFrame(data)
    ...
    graph.update_layout(title_text='Entradas Red', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    return graph

@app.callback(
    Output('value-entries-red', 'children'),
    [Input('store-reds', 'data')]
)
